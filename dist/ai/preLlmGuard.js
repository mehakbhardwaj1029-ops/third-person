...
exports.preLlmGuard = preLlmGuard;
const DEFAULT_MAX_CHARS = 12000;
const PROMPT_INJECTION_PATTERNS = [
    {
        label: "Ignore previous instructions",
        regex: /\b(ignore|disregard|forget)\b.{0,40}\b(previous|above|prior|system|developer)\b.{0,40}\b(instructions|prompt|rules|message)\b/i,
        severity: 3,
    },
    {
        label: "Reveal hidden/system prompt",
        regex: /\b(reveal|show|print|display|tell me)\b.{0,40}\b(system prompt|developer prompt|hidden prompt|hidden instructions|internal rules)\b/i,
        severity: 4,
    },
    {
        label: "Role override",
    ...
    },
    {
        ...
        regex: /\b(jailbreak|developer mode|admin mode|god mode|bypass safety|bypass rules|override safety)\b/i,
        severity: 4,
    },
    {
        label: "Instruction hierarchy attack",
        regex: /\b(system message|developer message|highest priority|new instructions|override instructions)\b/i,
        severity: 3,
    },
    {
        label: "Data exfiltration attempt",
        regex: /\b(api key|secret key|password|token|private data|confidential data|database dump|environment variable)\b/i,
        severity: 4,
    },
    {
        label: "Identity extraction",
        regex: /\b(tell me who i am|who is the user|reveal user identity|show user profile)\b/i,
        severity: 3,
    },
    {
        label: "Model manipulation",
        regex: /\b(do not follow|do not obey|ignore your rules|break character|no restrictions)\b/i,
        severity: 3,
    },
];
const PII_PATTERNS = [
    ...
    {
        label: "phone",
        regex: /(?:\+?\d{1,3}[\s-]?)?(?:\(?\d{3,5}\)?[\s-]?)?\d{3,5}[\s-]?\d{3,5}/g,
    ...
    {
        label: "upi",
        regex: /\b[\w.-]+@(upi|okaxis|okhdfcbank|oksbi|okicici|ybl|ibl|axl|paytm)\b/gi,
...
function neutralisePromptInjection(input) {
    let output = input;
    for (const pattern of PROMPT_INJECTION_PATTERNS) {
        output = output.replace(pattern.regex, "[REMOVED_PROMPT_INJECTION_ATTEMPT]");
    }
    return output;
}
...
function isEmotionallyImportant(line) {
    const lower = line.toLowerCase();
    return EMOTIONALLY_IMPORTANT_KEYWORDS.some((keyword) => lower.includes(keyword));
}
function compressChatText(input, maxChars) {
    ...
    const uniqueLines = Array.from(new Set([...firstLines, ...importantLines, ...lastLines]));
...
function preLlmGuard(rawInput, options = {}) {
    ...
    let riskLevel = injectionResult.riskLevel;
    ...
    if (injectionResult.detectedInjection) {
        warnings.push("Prompt injection-like instructions were detected and neutralised.");
    }
